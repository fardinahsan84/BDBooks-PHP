...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if($fname != "" && $lname != "" && $email != "" && $gender !="" && $uname != "" && $password != "" && $remail != "")
   {
    $data = array();
    $json_string = file_get_contents("../data/usersdb.json");
    $data = json_decode($json_string, true);
		array_push($data, array('firstName' => $fname, 'lastName' => $lname, 'email' => $email,'gender' => $gender, 
						'uname' => $uname,'password' => $password,'recoveryEmail' => $remail, 'type' => 'user'));
    $strNew = json_encode($data);
    file_put_contents("../data/usersdb.json", $strNew);
...
<ul>
  ...
  <li><a href="/BDBooks-PHP/book/allBooks.php">New Arrival</a></li>
...
<div class="hero-bg">
	<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
		...
		<a href ="/BDBooks-PHP/login.php">Sign in now!<a>
